...
double svlCCHandEyeCalibration::calibrate()
{
    ...
    for(int i=0;i<(int)calibrationGrids.size();i++)
    {
        for(int j=0;j<(int)calibrationGrids.size();j++)
        {
...
double svlCCHandEyeCalibration::optimizeDualQuaternionMethod()
{
    ...
    for(int j=0;j<(int)cameraMatrix.size();j++)
        myIndicies[j] = j;
...
void svlCCHandEyeCalibration::printData()
{
    ...
    for(int i=0;i<(int)worldToTCPMatrix.size();i++)
    {
