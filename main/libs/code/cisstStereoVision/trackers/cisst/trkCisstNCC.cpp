...
/*
  $Id: trkCisstNCC.cpp 821 2009-09-08 01:31:37Z adeguet1 $
...
trkCisstNCC::~trkCisstNCC(void) {
	resetTrack();
}
//Cear stored data.
void trkCisstNCC::resetTrack() {
...
void trkCisstNCC::setTemplateLookahead(IndexType number) {
	templateLookahead = number;
}
//Select the convolution method to use
void trkCisstNCC::setConvolutionMethodTo(ConvolutionMethod method) {
	convMethod = method;
}
//Set the size of the template. 
//All templates must be the same size.
void trkCisstNCC::setTemplateSize(IndexType rows, IndexType cols) {
	templateSize.X() = cols;
	templateSize.Y() = rows;
}
//Set the center offset of the template.
void trkCisstNCC::setTemplateCenter(IndexType r, IndexType c) {
	templateCenter.X() = c;
	templateCenter.Y() = r;
}
//Set the tracking window size
void trkCisstNCC::setWindowSize(IndexType rows, IndexType cols) {
	windowSize.X() = cols;
	windowSize.Y() = rows;
}
// Set the Image Size
void trkCisstNCC::setImageSize(IndexType rows, IndexType cols) {
	imageSize.X() = cols;
	imageSize.Y() = rows;
}
//Set the center offset of the tracking window
void trkCisstNCC::setWindowCenter(IndexType r, IndexType c) {
	windowCenter.X() = c;
	windowCenter.Y() = r;
}
...
void trkCisstNCC::copyImage_opencv(ImageType &src, MatrixType &dst, IndexType startR, IndexType startC) {
...
void trkCisstNCC::copyImage_opencv(MatrixType &src,ImageType &dst) {
...
void trkCisstNCC::pushTemplate_opencv(ImageType &src, IndexType r, IndexType c) {
	//Create a new matrix of the right size
	if(templateSize.X() <= 0 || templateSize.Y() <= 0){
		cmnThrow(std::runtime_error("trkCisstNCC pushtemplate_opencv - degenerate template size"));
	}
...
void trkCisstNCC::getTemplate_opencv(IndexType index, ImageType &dst) {
	if( index<0 && index >= getNumberOfTemplates() ) {
		cmnThrow(std::runtime_error("trkCisstNCC getTemplate_opencv - index invalid"));
	}
	copyImage_opencv(*allTemplates[index],dst);
}

//move the search window to the source image if near the edge (uses opencv)
void trkCisstNCC::checkRectangleBoundries_opencv(ImageType & src, PointType & size, PointType & position) {
...
void trkCisstNCC::updateTrack_opencv(ImageType  & src) {
...
void trkCisstNCC::copyImage_cisst(MatrixType &src, MatrixType &dst, IndexType startR, IndexType startC) {
	for(IndexType r = 0; r < dst.rows(); r++) {
		for(IndexType c = 0; c < dst.cols(); c++) {
...
void trkCisstNCC::copyCurrentImage(InterfaceImType *src, IndexType src_rows, IndexType src_cols){
	copyImage(src, src_rows, src_cols, Current_image);
}

//Unit normalize a template, source and destination must be seperate
void trkCisstNCC::NCC_NormalizeTemplate(MatrixType &src, MatrixType &dst) {
	//t-t_bar
...
void trkCisstNCC::pushTemplate(InterfaceImType *src, IndexType src_rows, IndexType src_cols, IndexType r, IndexType c) {
	//Create a new matrix of the right size
	if(templateSize.X() <= 0 || templateSize.Y() <= 0){
		cmnThrow(std::runtime_error("trkCisstNCC pushTemplate - degenerate template size"));
	}
	MatrixType * temp  = new MatrixType(templateSize.Y(), templateSize.X(), VCT_COL_MAJOR);
	MatrixType * temp2 = new MatrixType(templateSize.Y(), templateSize.X(), VCT_COL_MAJOR);
	//Copy in the image data
...
void trkCisstNCC::pushTemplate(InterfaceImType *src, IndexType src_rows, IndexType src_cols) {
	//Create a new matrix of the right size
	if(templateSize.X() <= 0 || templateSize.Y() <= 0){
		cmnThrow(std::runtime_error("trkCisstNCC pushTemplate - degenerate template size"));
	}
...
//void trkCisstNCC::getTemplate(IndexType index, MatrixType &dst) {
//	if( index<0 && index >= getNumberOfTemplates() ) {
...
void trkCisstNCC::pushSelectedTemplate(IndexType index) {
	selectedTemplates.push_back(allTemplates[index]);
}
//Remove the last selected template
void trkCisstNCC::popSelectedTemplate() {
...
void trkCisstNCC::checkRectangleBoundries(PointType & imSize, PointType & size, PointType & position) {
...
void trkCisstNCC::initializeTrack() {
	...
	if( templateSize.X()<=0 || templateSize.Y()<=0 ) {
		cmnThrow(std::runtime_error("trkCisstNCC initializeTrack() function - degenerate template size"));
	}
	if( windowSize.X() < templateSize.X() && windowSize.Y() < templateSize.Y() ) {
		cmnThrow(std::runtime_error("trkCisstNCC initializeTrack() function - window size should be larger than template size"));
	}
...
void trkCisstNCC::NCC_Energy(MatrixType &src, MatrixType &dst) {
...
void trkCisstNCC::NCC_Standard(MatrixType &srcI, MatrixType &srcT, MatrixType &energy, MatrixType &dst) {
