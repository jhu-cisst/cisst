...
bool mtsStateTable::AddFilter(mtsMonitorFilterBase * filter)
{
    ...
    mtsMonitorFilterBase::SignalNamesType inputNames = filter->GetNamesOfInputs();
    for (size_t i = 0; i < inputNames.size(); ++i) {
        if (GetStateVectorID(inputNames[i]) == INVALID_STATEVECTOR_ID) {
            CMN_LOG_CLASS_RUN_ERROR << "AddFilter: Input element \"" << inputNames[i] << "\" required by filter \""
    ...
    const int filterUID = filter->GetFilterUID();
    double * const outputs = filter->GetOutputs();

    // Add all of outputs of the filter to this state table
    std::stringstream ss;
    std::string newElementName;
    mtsMonitorFilterBase::SignalNamesType outputNames = filter->GetNamesOfOutputs();
    for (size_t i = 0; i < outputNames.size(); ++i) {
        ss << outputNames[i] << filterUID;
        newElementName = ss.str();
        NewElement(newElementName, &outputs[i]);
        CMN_LOG_CLASS_RUN_DEBUG << "AddFilter: Output element \"" << newElementName << "\" generated by filter \""
