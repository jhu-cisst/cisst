...
class CISST_EXPORT mtsSafetyCoordinator: public mtsGenericObject, public SC::Coordinator 
{
...
public:
    ...
    */
    ...
    typedef SC::Coordinator BaseType;
...
protected:
    ...
    bool DeployMonitorTarget(const std::string & targetJSON,
                             SC::cisstMonitor * cisstMonitorTargetInstance);

    //! Install monitoring targets from JSON
    /*! Called internally by AddMonitorTargetFromJSONFile() and AddMonitorTargetFromJSON() */
    bool AddMonitorTarget(const SC::JsonWrapper::JsonValue & targets);

    //! Install active filter
    bool AddFilterActive(SC::FilterBase * filter, mtsTask * targetComponent);
    //! Install passive filter
    bool AddFilterPassive(SC::FilterBase    * filter,
                          mtsTask           * targetComponent,
                          const std::string & targetProcessName,
                          const std::string & targetComponentName);

    //
    bool AddEvent(const SC::JsonWrapper::JsonValue & filters);
...
public:
    ...
    bool AddMonitorTarget(SC::cisstMonitor * monitor);
    ...
    bool AddFilter(SC::FilterBase * filter);
    ...
    /*! MJ: Right now, this method should be called main.cpp but could be moved
        into either cisst or SC such that users don't need to explicitly call this method.
    */
    ...
    //  Events
    //--------------------------------------------------
    // Called by casros (SC::Coordinator) when event is detected and generated by filter.
    ...
    bool OnEventHandler(const SC::Event * event);

    // TEMP: refer to casros/libs/supervisor/coorinator.h
    bool PublishMessage(SC::Topic::Control::CategoryType category, const std::string & msg);

    //--------------------------------------------------
    //  Misc.
    //--------------------------------------------------
    //! Endpoint to access casros network
    SC::cisstAccessor * GetCasrosAccessor(void) const { return casrosAccessor; }
    ...
    //! JSON serializer
    static const std::string GetJsonForPublish(const SC::cisstMonitor & monitorTarget,
