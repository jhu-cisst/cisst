...
int main(int argc, char * argv[])
{
    ...
    osaThreadedLogFile threadedLog("example5Client");
    ...
    cmnClassRegister::SetLoD("mainTask", CMN_LOG_LOD_VERY_VERBOSE);

    // create our server task
    const double PeriodClient = 1.0 * cmn_ms; // in milliseconds
    mainTask * mainTaskObject = new mainTask("MAIN", PeriodClient);


   
    // Get the TaskManager instance and set operation mode
    mtsTaskManager * taskManager = mtsTaskManager::GetInstance();
    taskManager->AddTask(mainTaskObject);        
    ...
    std::string omniName("DEFAULT");
    devNovintHDL * robotObject = new devNovintHDL("Falcon", "DEFAULT");
    ...
    taskManager->Connect("MAIN", "RobotMaster", "Falcon", "DEFAULT");
    taskManager->Connect("MAIN", "Button2", "Falcon", "DEFAULTButton2");
    taskManager->Connect("MAIN", "RobotSlave", "Omni", "Omni1");
    taskManager->Connect("MAIN", "Button4", "Omni", "Omni1Button2");
    ...
    mainTaskObject->Create();
    osaSleep(2.0 * cmn_s);
    robotObject->Create();
    osaSleep(2.0 * cmn_s);
    mainTaskObject->Start();
    osaSleep(2.0 * cmn_s);
    robotObject->Start();
    osaSleep(2.0 * cmn_s);
...
/*
  Author(s):  Gorkem Sevinc, Anton Deguet, Min Yang Jung
