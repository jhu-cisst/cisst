...
robLinearSE3::robLinearSE3( void ):
    IsSet( false )
{}
...
robLinearSE3::robLinearSE3( const vctFrame4x4<double>& Rtw1,
                            ...
                            double t1 )
{
    Set(Rtw1, Rtw2, vmax, wmax, t1);
}


void robLinearSE3::Set(const vctFrame4x4<double> &Rtw1,
                       const vctFrame4x4<double> &Rtw2,
                       double vmax, double wmax, double t1)
{
    IsSet = false;
    this->vmax = vmax;
    this->wmax = wmax;

    // set time, start & stop pose
    robFunction::Set(t1, t1);
    this->Rtw1 = Rtw1;
    this->v1w1 = vctFixedSizeVector<double,6>( 0.0 );
    this->v1dw1d = vctFixedSizeVector<double,6>( 0.0 );

    this->Rtw2 = Rtw2;
    this->v2w2 = vctFixedSizeVector<double,6>( 0.0 );
    this->v2dw2d = vctFixedSizeVector<double,6>( 0.0 );

    // compute Rtw2 w.r.t. Rtw1
    ...
    double tduration = t12.Norm() / vmax;     // translation duration
    double rduration = r12.Angle() / wmax;    // rotation duration
    ...
    if (translation != NULL) delete translation;  // delete old translation
    ...
    if (rotation != NULL) delete rotation;   // delete old rotation
...
void robLinearSE3::Evaluate( double t, 
    ...
    // put position + rotation together
