...
robSAH::robSAH( const std::string& palmfilename,
  ...
  tool.push_back( new robSAHThumb( Rtwp ) );
...
robSAH::ForwardKinematics( const vctFixedSizeVector<double,3>& q, 
  ...
  switch( finger ){
  case robSAH::FIRST: 
    sahfinger = dynamic_cast<const robSAHFinger*>( tool[1] );
    break;

  case robSAH::MIDDLE:
    sahfinger = dynamic_cast<const robSAHFinger*>( tool[2] );
    break;

  case robSAH::RING:
    sahfinger = dynamic_cast<const robSAHFinger*>( tool[3] );
    break;
...
robSAH::ForwardKinematics( const vctFixedSizeVector<double,4>& q, 
  ...
  const robSAHThumb* thumb = dynamic_cast<const robSAHThumb*>( tool[0] );
  if( thumb != NULL )
