...
void robCollaborativeControlForce::Update(const prmPositionCartesianGet & p1, 
                                          ...
                                          vctFixedSizeVector<double, 6> & ForceSlave)
{
    ...
    if((ClutchMaster == true || MasterClutch == true) 
        ...
        //DevicesVector(SlaveDeviceNo)->ForceCartesian.SetForce(ForceSlave);
        ForceMaster.SetAll(0.0);
        //DevicesVector(MasterDeviceNo)->ForceCartesian.SetAll(0.0, true);
        
        // Compute the offset of the master device from its original position
        LeftClutchOffset.DifferenceOf(p1.Position().Translation(), p1Last.Position().Translation());
        ...
        //DevicesVector(MasterDeviceNo)->ForceCartesian.SetForce(ForceMaster);
        //DevicesVector(SlaveDeviceNo)->ForceCartesian.SetAll(0.0, true);
        ForceSlave.SetAll(0.0);
        
        // Compute the offset of the slave device from its original position
        //p2Clutched = GetPosition(SlaveDeviceNo);
        RightClutchOffset.DifferenceOf(p2.Position().Translation(), p2Last.Position().Translation());
        ...
        LeftClutchMSOffset.DifferenceOf(p1.Position().Translation(), p1Last.Position().Translation());
        RightClutchMSOffset.DifferenceOf(p2.Position().Translation(), p2Last.Position().Translation());
...
void robCollaborativeControlForce::SetParameters(const mtsDouble & commandedForceLimit, const mtsDouble & commandedScaleFactor, 
