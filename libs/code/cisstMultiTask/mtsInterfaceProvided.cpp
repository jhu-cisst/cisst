    ...
    PostCommandQueuedCallable(originalInterface->PostCommandQueuedCallable)
{
    ...
    for (;
        ...
        CommandsVoid.AddItem(iterVoid->first, commandVoid, CMN_LOG_BIT_INIT_ERROR);
    ...
    for (;
        ...
        CommandsVoidReturn.AddItem(iterVoidReturn->first, commandVoidReturn, CMN_LOG_BIT_INIT_ERROR);
    ...
    for (;
        ...
        CommandsWrite.AddItem(iterWrite->first, commandWrite, CMN_LOG_BIT_INIT_ERROR);
    }
    ...
    for (;
        ...
        CommandsWriteReturn.AddItem(iterWriteReturn->first, commandWriteReturn, CMN_LOG_BIT_INIT_ERROR);
                                                      ...
                                                      mtsCommandQueueingPolicy queueingPolicy)
{
    ...
    if (callable) {
        ...
        if (!queued) {
            ...
            if (!CommandsVoid.AddItem(name, command, CMN_LOG_BIT_INIT_ERROR)) {
                delete command;
            ...
            if (!CommandsVoid.AddItem(name, queuedCommand, CMN_LOG_BIT_INIT_ERROR)) {
...
mtsCommandVoid * mtsInterfaceProvided::AddCommandVoid(mtsCommandVoid * command)
{
    // check that the input is valid
    if (command) {
        if (!CommandsVoid.AddItem(command->GetName(), command, CMN_LOG_BIT_INIT_ERROR)) {
            CMN_LOG_CLASS_INIT_ERROR << "AddCommandVoid: unable to add command \""
                                     << command->GetName() << "\"" << std::endl;
        }
        return command;
                                                                  ...
                                                                  mtsCommandQueueingPolicy queueingPolicy)
{
    ...
    if (callable) {
        ...
        if (!queued) {
            ...
            if (!CommandsVoidReturn.AddItem(name, command, CMN_LOG_BIT_INIT_ERROR)) {
                delete command;
            ...
            if (!CommandsVoidReturn.AddItem(name, queuedCommand, CMN_LOG_BIT_INIT_ERROR)) {
                delete queuedCommand;
                queuedCommand = 0;
...
mtsCommandVoidReturn * mtsInterfaceProvided::AddCommandVoidReturn(mtsCommandVoidReturn * command)
{
    // check that the input is valid
    if (command) {
        if (!CommandsVoidReturn.AddItem(command->GetName(), command, CMN_LOG_BIT_INIT_ERROR)) {
            CMN_LOG_CLASS_INIT_ERROR << "AddCommandVoidReturn: unable to add command \""
                                     << command->GetName() << "\"" << std::endl;
        }
        return command;
...
mtsCommandWriteBase * mtsInterfaceProvided::AddCommandWrite(mtsCommandWriteBase * command, mtsCommandQueueingPolicy queueingPolicy)
{
    ...
    if (command) {
        ...
        if (!queued) {
            if (!CommandsWrite.AddItem(command->GetName(), command, CMN_LOG_BIT_INIT_ERROR)) {
                command = 0;
            ...
            if (!CommandsWrite.AddItem(command->GetName(), queuedCommand, CMN_LOG_BIT_INIT_ERROR)) {
                delete queuedCommand;
                queuedCommand = 0;
                                                                    ...
                                                                    mtsCommandQueueingPolicy queueingPolicy)
{
    ...
    if (callable) {
        ...
        if (!queued) {
            ...
            if (!CommandsWriteReturn.AddItem(name, command, CMN_LOG_BIT_INIT_ERROR)) {
                delete command;
            ...
            if (!CommandsWriteReturn.AddItem(name, queuedCommand, CMN_LOG_BIT_INIT_ERROR)) {
                delete queuedCommand;
                queuedCommand = 0;
                                                      ...
                                                      mtsGenericObject * argumentPrototype)
{
    ...
    if (callable) {
        ...
        if (!CommandsRead.AddItem(name, command, CMN_LOG_BIT_INIT_ERROR)) {
            delete command;
...
{
    if (command) {
        if (!CommandsRead.AddItem(command->GetName(), command, CMN_LOG_BIT_INIT_ERROR)) {
            command = 0;
            CMN_LOG_CLASS_INIT_ERROR << "AddCommandRead: unable to add command \""
                                     << command->GetName() << "\"" << std::endl;
        }
...
{
    if (command) {
        if (!CommandsQualifiedRead.AddItem(command->GetName(), command, CMN_LOG_BIT_INIT_ERROR)) {
            command = 0;
...
{
    if (filter && command) {
        if (!CommandsInternal.AddItem(filter->GetName(), filter, CMN_LOG_BIT_INIT_ERROR)) {
        ...
        if (!CommandsInternal.AddItem(command->GetName()+"Write", command, CMN_LOG_BIT_INIT_ERROR)) {
            CMN_LOG_CLASS_INIT_ERROR << "AddCommandFilteredWrite: unable to add command \""
                                     << command->GetName() << "\"" << std::endl;
            CommandsInternal.RemoveItem(filter->GetName(), CMN_LOG_BIT_INIT_ERROR);
            return 0;
        }
        ...
        if (filteredCommand && CommandsWrite.AddItem(command->GetName(), filteredCommand, CMN_LOG_BIT_INIT_ERROR)) {
            return filteredCommand;
        } else {
            CommandsInternal.RemoveItem(filter->GetName(), CMN_LOG_BIT_INIT_ERROR);
            CommandsInternal.RemoveItem(command->GetName(), CMN_LOG_BIT_INIT_ERROR);
            if (filteredCommand) {
...
mtsCommandVoid * mtsInterfaceProvided::GetCommandVoid(const std::string & commandName) const {
    if (this->EndUserInterface) {
        return CommandsVoid.GetItem(commandName, CMN_LOG_BIT_INIT_ERROR);
    }
...
mtsCommandVoidReturn * mtsInterfaceProvided::GetCommandVoidReturn(const std::string & commandName) const {
    if (this->EndUserInterface) {
        return CommandsVoidReturn.GetItem(commandName, CMN_LOG_BIT_INIT_ERROR);
    }
...
mtsCommandWriteBase * mtsInterfaceProvided::GetCommandWrite(const std::string & commandName) const {
    if (this->EndUserInterface) {
        return CommandsWrite.GetItem(commandName, CMN_LOG_BIT_INIT_ERROR);
    }
...
mtsCommandWriteReturn * mtsInterfaceProvided::GetCommandWriteReturn(const std::string & commandName) const {
    if (this->EndUserInterface) {
        return CommandsWriteReturn.GetItem(commandName, CMN_LOG_BIT_INIT_ERROR);
    }
...
mtsCommandRead * mtsInterfaceProvided::GetCommandRead(const std::string & commandName) const {
    ...
    return CommandsRead.GetItem(commandName, CMN_LOG_BIT_INIT_ERROR);
}
...
mtsCommandQualifiedReadBase * mtsInterfaceProvided::GetCommandQualifiedRead(const std::string & commandName) const {
    ...
    return CommandsQualifiedRead.GetItem(commandName, CMN_LOG_BIT_INIT_ERROR);
}
...
mtsMulticastCommandVoid * mtsInterfaceProvided::GetEventVoid(const std::string & eventName) const {
    ...
    return EventVoidGenerators.GetItem(eventName, CMN_LOG_BIT_INIT_ERROR);
}
...
mtsMulticastCommandWriteBase * mtsInterfaceProvided::GetEventWrite(const std::string & eventName) const {
    ...
    return EventWriteGenerators.GetItem(eventName, CMN_LOG_BIT_INIT_ERROR);
}
