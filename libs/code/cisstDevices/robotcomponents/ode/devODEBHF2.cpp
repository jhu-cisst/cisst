...
const double devODEBHF2::TRQMAX = 1.0;
const double devODEBHF2::VELMAX = 1.0;

devODEBHF2::devODEBHF2( const std::string& devname,
			double period,
  ...
  qinit = vctDynamicVector<double>( 3, 0.0, 0.0, cmnPI/4.0 );
  ...
  metacarp = new devODEBody( world.WorldID(),                       // world
			     spaceid,                               // space
			     f2.ForwardKinematics( q, robBHF2::METACARP ),
			     f2.links[0].Mass(),                    // m   
			     f2.links[0].CenterOfMass(),            // com
			     f2.links[0].MomentOfInertiaAtCOM(),    // I  
  ...
  mcp1 = new devODEJoint( world.WorldID(),            // the world ID
			  ...
			  0.0,                        // the lower limit
			  f2.links[0].PositionMax() -
			  f2.links[0].PositionMin() );// the upper limit
  this->Insert( mcp1 );
  world.Insert( mcp1 );
  ...
  proximal = new devODEBody( world.WorldID(),                       // world
			     spaceid,                               // space
			     f2.ForwardKinematics( q, robBHF2::PROXIMAL ),
			     f2.links[1].Mass(),                    // m   
			     f2.links[1].CenterOfMass(),            // com
			     f2.links[1].MomentOfInertiaAtCOM(),    // I  
			     proximalgeom );
  this->Insert( proximal );
  world.Insert( proximal );

  // Create and initialize the second joint (MCP)
  Rtwi = f2.ForwardKinematics( q, robBHF2::METACARP );
  mcp2 = new devODEJoint( world.WorldID(),            // the world ID
			  ...
			  Rtwi.Rotation() * z,        // the Z axis 
			  0.0,                        // the lower limit
			  f2.links[1].PositionMax() -
			  f2.links[1].PositionMin() );// the upper limit
  this->Insert( mcp2 );
  world.Insert( mcp2 );
  ...
  intermediate = new devODEBody( world.WorldID(),                   // world
				 spaceid,                           // space
				 f2.ForwardKinematics(q, robBHF2::INTERMEDIATE),
				 f2.links[2].Mass(),                // m   
				 f2.links[2].CenterOfMass(),        // com
				 f2.links[2].MomentOfInertiaAtCOM(),// I  
				 intermediategeom );
  this->Insert( intermediate );
  world.Insert( intermediate );

  // Create and initialize the second joint (PIP)
  Rtwi = f2.ForwardKinematics( q, robBHF2::PROXIMAL );
  pip = new devODEJoint( world.WorldID(),             // the world ID
			 ...
			 Rtwi.Rotation() * z,         // the Z axis 
			 0.0,                         // the lower limit
			 f2.links[2].PositionMax() -
			 f2.links[2].PositionMin() ); // the upper limit
  this->Insert( pip );
  world.Insert( pip );
...
void devODEBHF2::Write( const vctDynamicVector<double>& qs ){
