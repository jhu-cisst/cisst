...
devGLUT* devGLUT::glut = NULL;

// keyboard callback function
void KeyboardFunc(unsigned char c, int x, int y)
{ devGLUT::glut->Keyboard( (int)c, x, y ); }

// special key callback function
void SpecialFunc(int c, int x, int y)
{ devGLUT::glut->Keyboard( c, x, y ); }
...
void devGLUT::Refresh()
{ glutPostRedisplay(); }

// compute the camera position
vctFixedSizeVector<double,3> devGLUT::CameraPosition() const {
...
// draw the floor
void devGLUT::DrawGrid(double width, int subdivision){
  glLineWidth(1);
...
devGeometry* devGLUT::LoadOBJ( const std::string& filename ){
  devMeshTriangular* trimesh = new devMeshTriangular();
  trimesh->LoadOBJ( filename );
  Register(trimesh);
  return trimesh;
}

devGLUT::devGLUT( int argc, char** argv){
