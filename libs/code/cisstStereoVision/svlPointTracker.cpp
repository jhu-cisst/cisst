...
/*
  $Id: svlPointTracker.cpp,v 1.13 2008/10/22 21:46:26 vagvoba Exp $
...
/******************************************/
/*** svlPointTracker class ****************/
/******************************************/

svlPointTracker::svlPointTracker() : svlFilterBase()
{
...
svlPointTracker::~svlPointTracker()
{
...
void svlPointTracker::SetMovingAverageSmoothing(double weight)
{
    if (weight < 0.0) MovingAverageWeight = 0.0;
    else MovingAverageWeight = weight;
}

int svlPointTracker::SetTrackerAlgo(svlPointTrackerAlgoBase* tracker, unsigned int targetcount, unsigned int videoch)
{
...
int svlPointTracker::SetWorkArea(unsigned int left, unsigned int top, unsigned int right, unsigned int bottom, unsigned int videoch)
{
    if (GetInputType() == svlTypeImageRGB) videoch = SVL_LEFT;
    else if (videoch > 1) return SVL_FAIL;
    if (IsInitialized()) return SVL_ALREADY_INITIALIZED;
...
int svlPointTracker::SetTarget(unsigned int targetid, int x, int y, unsigned int videoch)
{
    if (GetInputType() == svlTypeImageRGB) videoch = SVL_LEFT;
    else if (videoch > 1) return SVL_FAIL;
...
void svlPointTracker::SetIterations(unsigned int count)
{
    if (count < 1) count = 1;
...
int svlPointTracker::Initialize(svlSample* inputdata)
{
...
int svlPointTracker::ProcessFrame(ProcInfo* procInfo, svlSample* inputdata)
{
...
int svlPointTracker::AllocateTargets(unsigned int targetcount, unsigned int videoch)
{
...
void svlPointTracker::ReleaseTargets(unsigned int videoch)
{
    if (Target[videoch]) {
...
void svlPointTracker::ResetToForcedPositions(unsigned int videoch)
{
    svlSamplePointCloud* points = dynamic_cast<svlSamplePointCloud*>(OutputData);
...
int svlPointTracker::StoreResults(unsigned int videoch)
{
    svlSamplePointCloud* points = dynamic_cast<svlSamplePointCloud*>(OutputData);
    TargetType target;
...
int svlPointTrackerAlgoBase::SetTarget(unsigned int targetid, svlPointTracker::TargetType* target)
{
...
int svlPointTrackerAlgoBase::GetTarget(unsigned int targetid, svlPointTracker::TargetType* target)
{
...
void svlPointTrackerAlgoBase::ReallocateTargetParams(unsigned int targetcount)
{
    ...
    if (targetcount > 0) {
        TargetParams = new svlPointTracker::TargetType[targetcount];
        memset(TargetParams, 0, sizeof(svlPointTracker::TargetType));
    }
}
