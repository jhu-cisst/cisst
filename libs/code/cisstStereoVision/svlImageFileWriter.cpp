...
/*
  $Id: svlImageFileWriter.cpp,v 1.13 2008/10/22 20:04:13 vagvoba Exp $
...
int svlImageFileWriter::Initialize(svlSample* inputdata)
{
...
int svlImageFileWriter::ProcessFrame(ProcInfo* procInfo, svlSample* inputdata)
{
...
int svlImageFileWriter::Release()
{
    for (int i = 0; i < 2; i ++) {
...
int svlImageFileWriter::Disable(bool disable, int videoch)
{
...
int svlImageFileWriter::SetFilePath(const char* filepathprefix, const char* extension, int videoch)
{
    if (IsInitialized() == true)
