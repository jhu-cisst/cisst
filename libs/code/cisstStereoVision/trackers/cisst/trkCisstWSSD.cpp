...
/*
  $Id: trkCisstWSSD.cpp,v 1.3 2007/09/27 00:14:03 vagvoba Exp $
...
trkCisstWSSD::~trkCisstWSSD(void) {
	resetTrack();
}
//Clear stored data.
void trkCisstWSSD::resetTrack() {
...
void trkCisstWSSD::setTemplateLookahead(IndexType number) {
	templateLookahead = number;
}
//Set the size of the template. 
//All templates must be the same size.
void trkCisstWSSD::setTemplateSize(IndexType rows, IndexType cols) {
	templateSize.X() = cols;
	templateSize.Y() = rows;
}
...
void trkCisstWSSD::setCurrentimageSize(IndexType rows, IndexType cols) {
	Current_image.SetSize(rows,cols, VCT_COL_MAJOR);
}
// Set the Image Size
void trkCisstWSSD::setImageSize(IndexType rows, IndexType cols) {
	imageSize.X() = cols;
	imageSize.Y() = rows;
}
...
void trkCisstWSSD::copyImage_opencv(ImageType &src, MatrixType &dst, IndexType startR, IndexType startC) {
...
void trkCisstWSSD::copyImage_opencv(MatrixType &src,ImageType &dst) {
...
void trkCisstWSSD::pushTemplate_opencv(ImageType &src, IndexType r, IndexType c) {
	//Create a new matrix of the right size
	if(templateSize.X() <= 0 || templateSize.Y() <= 0){
		cmnThrow(std::runtime_error("trkCisstWSSD initializeTrack() function - degenerate template size"));
	}
	...
	WeightedSSD_NormalizeTemplate(*temp,*temp2);
...
void trkCisstWSSD::getTemplate_opencv(IndexType index, ImageType &dst) {
	if( index<0 && index >= getNumberOfTemplates() ) {
		cmnThrow(std::runtime_error("trkCisstWSSD getTemplate_opencv function - incorrect index number"));
	}
	copyImage_opencv(*allTemplates[index],dst);
}

//Update the current window position
void trkCisstWSSD::updateTrack_opencv(ImageType &  src) {
	
	copyImage(src,imagecopy);
	WarpedMatrix.SetRef(templateSize.Y(),templateSize.X(),Current_image);

	if(selectedTemplaes.size() == 1)
	{
	...
	else
	{
		for(IndexType i = 0; i < numIter; i++)
		{
			...
			for(IndexType i = 0; i < derCurrent_region.size(); i++)
			{
				...
				tempVec1.SetRef(current_region.size(),(*derCurrent_region[i]).Pointer());
...
void trkCisstWSSD::copyImage_cisst(MatrixType &src, MatrixType &dst, IndexType startR, IndexType startC) {
	for(IndexType r = 0; r < dst.rows(); r++) {
		for(IndexType c = 0; c < dst.cols(); c++) {
...
void trkCisstWSSD::copyCurrentImage(InterfaceImType *src, IndexType src_rows, IndexType src_cols){
	copyImage(src, src_rows, src_cols, Current_image);
}

// Add another template to the tracker (from the src image)
void trkCisstWSSD::pushTemplate(InterfaceImType *src, IndexType src_rows, IndexType src_cols, IndexType r, IndexType c) {
	//Create a new matrix of the right size
	if(templateSize.X() <= 0 || templateSize.Y() <= 0){
...
void trkCisstWSSD::pushTemplate(InterfaceImType *src, IndexType src_rows, IndexType src_cols) {
	//Create a new matrix of the right size
	if(templateSize.X() <= 0 || templateSize.Y() <= 0){
		cmnThrow(std::runtime_error("trkCisstWSSD pushTemplate function - degenerate template size"));
	}
...
void trkCisstWSSD::popSelectedTemplate() {
...
void trkCisstWSSD::setThreshold(RealType thres) {
	threshold = thres;
}

//Cisst Library Test Routines
//void trkCisstWSSD::test() {}

void trkCisstWSSD::computeXderivative(MatrixType &src, MatrixType &dst)
{
...
void trkCisstWSSD::computeYderivative(MatrixType &src, MatrixType &dst)
{
...
void trkCisstWSSD::computeXYvalues(MatrixType &x_vals, MatrixType &y_vals)
{
...
void trkCisstWSSD::computeTemplateDerivative(IndexType index){
...
void trkCisstWSSD::computeCurrentDerivative(){
	// Note the derivatives are computed for the list of selected templates
	// compute x derivative
...
void trkCisstWSSD::copyCurrentRegion()
{
	// Copy current region 
...
void trkCisstWSSD::initializeTrack() {
	...
	if( templateSize.X()<=0 || templateSize.Y()<=0 ) {
		cmnThrow(std::runtime_error("trkCisstWSSD initializeTrack - degenerate template size"));
	}
