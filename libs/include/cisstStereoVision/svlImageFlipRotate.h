...
/*
  $Id: svlImageFlipRotate.h,v 1.3 2008/10/17 17:44:38 vagvoba Exp $
...
class CISST_EXPORT svlImageFlipRotate : public svlFilterBase
{
public:
    svlImageFlipRotate();
    virtual ~svlImageFlipRotate();
...
template <class _pixelType>
void svlImageFlipRotate::FlipRotate(_pixelType* input, _pixelType* output, const unsigned int inwidth, const unsigned int inheight,
                                    const int quickcopy, const int outstart, const int outstride, const int outlinestride)
{
    if (quickcopy == 0) {
        unsigned int i, j;
